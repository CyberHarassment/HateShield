...
import torch
import pickle
from data.datasets import HateMMLanguagebindDataset, GAP
import numpy as np
...
def pred_per_video(video_data, query, show_info=True, sim_thres=0.65, num_modals_thres=2, num_clips_thres=2):
    ...
    # all_sim_positives = combined_sim[:,1].unsqueeze(1)
...
def test_fold(data_dir, all_data_dict, fold, sim_thres=0.65, num_modals_thres=2, num_clips_thres=2, save_pred=None):
    ...
    trainset = HateMMLanguagebindDataset(data_dir, data_dict, split='train', preprocess=GAP())
    testset = HateMMLanguagebindDataset(data_dir, data_dict, split='test')
    ...
    for i in progress_bar:
        ...
        save_pred[name]['seg'] = seg.cpu().numpy()
...
if __name__ == '__main__':
    ...
    data_dir = 'path to the processed features of LanguageBind'
    data_file = './data/crossval_gt.pkl'
    all_data_dict = pickle.load(open(data_file, 'rb'))
    folds = ['fold1', 'fold2', 'fold3', 'fold4', 'fold5']
    save_file = './data/HateMM_LAHVD_languagebind.pkl'

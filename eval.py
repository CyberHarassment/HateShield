...
gt = pickle.load(open('data/eval_gt.pkl', 'rb'))
pred = pickle.load(open('data/HateMM_clipclap.pkl', 'rb'))
...
for key in keys:
    ...
    all_labels.append(gt[key]['label'])


    pred_seg = pred[key]['seg']
    gt_seg = gt[key]['duration']
    ...
    if len(pred_seg) < len(gt_seg):
        pred_seg = np.append(pred_seg, np.zeros(len(gt_seg) - len(pred_seg)))
